...
export default class InvitesHandler {
  ...
  async getInviteParams(contactID: string) {
    ...
    const contactInfo = await contact.getInfo()

    if (!contactInfo.profile.ethAddress) {
      throw new Error('No eth address found for recipient')
    }
    const toAddrHash = hash(bufferFromHex(contactInfo.profile.ethAddress))
    const toFeedHash = hash(Buffer.from(contactInfo.publicID))
    return [toAddrHash, toFeedHash, this.user.getPublicID()]
  }

  async processInviteTransaction(contactID: Contact) {
  ...
  async sendInviteTX(contactID: string): Promise<void> {
    ...
    try {
      ...
      const invite = {
        ...
        toAddress: contactInfo.profile.ethAddress,
        stakeState: 'staked',
